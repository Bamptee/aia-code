...
export async function runStep(step, feature, { description, instructions, history, model: modelOverride, verbose = false, apply = false, root = process.cwd(), onData } = {}) {
  ...
  try {
    ...
    const prompt = await buildPrompt(feature, step, { description, instructions, history, root });
